...
ECHO=echo
VERBOSE=":"
STRIP="/cygdrive/i/MINGW/BIN/strip"
CMP=cmp
...
do
    ...
    -fifo)	CMD=@MKFIFO@; HasSource=no;
		...
		STRIP=":"
		;;
    -ln)	shift; CMD=cp -fp; SRC="$1"
		STRIP=":"
		;;
    -lns)	shift; CMD=cp -fp; preopts=""; SRC="$1"
		STRIP=":"
		;;
    -src)	shift; SRC="$1";;
    -[fF])	shift; DESTDIR="$1/";;
...
install()
{
	...
	$VERBOSE "$MV $target $DESTDIROLD$bf"
	$MV $target $DESTDIROLD$bf
    fi
