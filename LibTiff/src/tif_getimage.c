...
// Soft Gems: modification for GraphicEx: R and B are reversed on Windows.
#define A1 (((uint32)0xffL)<<24)
#define	PACK(b,g,r)	\
	((uint32)(r)|((uint32)(g)<<8)|((uint32)(b)<<16)|A1)
#define	PACK4(b,g,r,a)	\
	((uint32)(r)|((uint32)(g)<<8)|((uint32)(b)<<16)|((uint32)(a)<<24))
#define W2B(v) (((v)>>8)&0xff)
#define	PACKW(b,g,r)	\
	((uint32)W2B(r)|((uint32)W2B(g)<<8)|((uint32)W2B(b)<<16)|A1)
#define	PACKW4(b,g,r,a)	\
...
/*
 * 16-bit packed CMYK samples w/o Map => RGB
 *
 * NB: The conversion of CMYK->RGB is *very* crude.
 */
...
/*
 * 16-bit packed CMYK samples w/Map => RGB
 *
 * NB: The conversion of CMYK->RGB is *very* crude.
 */
DECLAREContigPutFunc(putRGBcontig16bitCMYKMaptile)
{
    ...
    while (h-- > 0) {
      for (x = w; x-- > 0;) {
          k = 0xffff - run[3];
          r = (k * (0xffff - run[0])) / 0xffff;
          g = (k * (0xffff - run[1])) / 0xffff;
          b = (k * (0xffff - run[2])) / 0xffff;
    ...
    if (buildMap(img)) {
      ...
      case PHOTOMETRIC_SEPARATED:
          switch (img->bitspersample)
          {
            ...
            case 16:
              if (!img->Map)
                  put = putRGBcontig16bitCMYKtile;
              else
                  put = putRGBcontig16bitCMYKMaptile;
              break;
          }
          break;
      case PHOTOMETRIC_PALETTE:
