...
begin
  if not DoControlMessage(Control, Message) then
  begin
    if (ThemeServices <> nil) and ThemeServices.ThemesEnabled and TPanelCast(Control).ParentColor or
...
function TThemeManager.CreateDisabledBitmap(FOriginal: TBitmap; OutlineColor: TColor): TBitmap;
...
var
  ...
  GlyphCount: Integer;
  GlyphMask, GlyphDisabled: TBitmap;
  Offset: TPoint;
...
begin
  ...
  begin
    ...
    with Glyph do
      if not Empty then
      begin
        // 25.8.2004: Ervin: For BitBtn controls I use old algoritem for grayed image on button
        if ((Control is TBitBtn) or (Control is TSpeedButton)) and not(TControlCast(Control).Enabled) and
          (GlyphCount = 1) then
        begin
          GlyphDisabled := TBitmap.Create;
          GlyphMask := TBitmap.Create;
          try
            GlyphDisabled.Assign(CreateDisabledBitmap(Glyph, clBtnFace));
            GlyphMask.Assign(GlyphDisabled);
            GlyphMask.Mask(GlyphDisabled.TransparentColor);
            TransparentStretchBlt(DC, GlyphPos.X, GlyphPos.Y, GlyphWidth, Height, GlyphDisabled.Canvas.Handle,
              GlyphSourceX, 0, GlyphWidth, Height, GlyphMask.Canvas.Handle, GlyphSourceX, 0);
          finally
            FreeAndNil(GlyphDisabled);
            FreeAndNil(GlyphMask);
          end;
        end
        else
        begin
...
begin
  ...
  if IsWindowsXP then
  begin
    // If we are inserting a new control but themes are not enabled then do nothing except the
    // control is a listview.
    if Inserting and (ThemeServices <> nil) and (not ThemeServices.ThemesEnabled) then
    begin
...
begin
  if not (csDesigning in ComponentState) and not Finalized then
  begin
    case Operation of
      opInsert:
