...
//----------------------------------------------------------------------------------------------------------------------
// 15-JUN-2005 edj:
//              Modifications to allow GeoTIFF subclass to access proper tags and types
//                          added TShortArray and TDoubleArray types
//                          added GetValueList for TShortArray and TDoubleArray
//              Made all TIFF tag access calls public for access from other units/classes
//              Added support for tiled TIFFs to TTiffGraphic.ReadFromStream
//                          moved some logic from ReadFromStream into separate procedures and methods }
// edj is Erik Johnson, erik@edj.net
...
type
  ...
  private
    ...
    procedure SortIFD;
    procedure SwapIFD;
    PROCEDURE SetupColorConversion;
    FUNCTION CreateDecompressor( Stream : TStream ) : TDecoder;
...
//    procedure SaveToStream(Stream: TStream); override;
    ...
    procedure GetValueList(Stream: TStream; Tag: Cardinal; var Values: TShortArray); overload;
    procedure GetValueList(Stream: TStream; Tag: Cardinal; var Values: TDoubleArray); overload;
  end;
...
procedure TTIFFGraphic.GetValueList(Stream: TStream; Tag: Cardinal; var Values: TShortArray);
...
procedure TTIFFGraphic.GetValueList(Stream: TStream; Tag: Cardinal; var Values: TDoubleArray);
// returns the values of the IFD entry indicated by Tag
var
  Index,
  Shift,
...
//----------------------------------------------------------------------------------------------------------------------
   ...
   PROCEDURE TTIFFGraphic.SetupColorConversion;
    BEGIN
   ...
   FUNCTION TTIFFGraphic.CreateDecompressor( Stream : TStream ) : TDecoder;
    BEGIN // create decompressor for the image
      Result := NIL;
      WITH FImageProperties DO case Compression of
        ctNone:;
        {$ifdef UseLZW}
        ctLZW:        Result := TTIFFLZWDecoder.Create;
        {$endif}
        ctPackedBits: Result := TPackbitsRLEDecoder.Create;
        ctFaxRLE,
        ctFaxRLEW:    Result := TCCITTMHDecoder.Create(GetValue(TIFFTAG_GROUP3OPTIONS), ioReversed in Options, Compression = ctFaxRLEW, Width);
        ctFax3:       Result := TCCITTFax3Decoder.Create(GetValue(TIFFTAG_GROUP3OPTIONS), ioReversed in Options, False, Width);
        ctJPEG:     begin // some extra work is needed for JPEG
                      GetValueList(Stream, TIFFTAG_JPEGTABLES, JPEGTables);
                      Result := TTIFFJPEGDecoder.Create(@FImageProperties);
                    end;
        ctThunderscan: Result := TThunderDecoder.Create(Width);
        ctLZ77:        Result := TLZ77Decoder.Create(Z_PARTIAL_FLUSH, True);
      else
       {COMPRESSION_OJPEG,
      ...
      if Assigned(Result) then Result.DecodeInit;
    END;


procedure TTIFFGraphic.LoadFromStream(Stream: TStream);
 var StripSize: Cardinal;
     Decoder: TDecoder;
     Deprediction: procedure(P: Pointer; Count: Cardinal); // dynamically assigned handler
     Offsets,
     ByteCounts: TCardinalArray;

   procedure SetupColorPalette;
    var ColorMap : Cardinal;
    begin
      WITH FImageProperties DO
       BEGIN
         FProgressRect.Right := Width;
         if ColorManager.TargetColorScheme in [csIndexed, csG, csGA] then
         begin // load palette data and build palette
   ...
   procedure DeterminePredictionScheme;
    begin
      with FImageProperties DO IF Compression <> ctNone then
        begin
   ...
   procedure ReadStrips;
    VAR Buffer, EncodedData, DataPointerCopy : Pointer;
        Pixels : Pointer;
        Run : PChar;
    BEGIN
      WITH FImageProperties DO
       BEGIN // go for each strip in the image (which might contain more than one line)
        CurrentRow := 0;
        CurrentStrip := 0;
        ...
        begin
          ...
          Run := Buffer;
          try // decompress strip if necessary
   ...
   procedure ReadTiles;
    ...
    BEGIN
      WITH FImageProperties DO
       BEGIN // go for each strip in the image (which might contain more than one line)
        CurrentRow := 0; CurrentStrip := 0;
        MasterRow := 0;
        TileX := 0;      TileY := 0;
        TileCols := Width DIV TileWidth;
        TileRows := Height DIV TileLength;
        StripCount := Length(Offsets);
        while CurrentStrip < StripCount do
        begin
          { determine tile index }
          TileX := CurrentStrip MOD TileRows;
          TileY := CurrentStrip DIV TileCols;
          MasterRow := TileY * TileLength;
          ...
          Run := Buffer;
          try
             if Assigned(Decoder) then // decompress strip if necessary
              begin
                GetMem(EncodedData, ByteCounts[CurrentStrip]);
                try
                   DataPointerCopy := EncodedData;
             ...
             Run := Buffer;
             // go for each line (row) in the strip
             DBGWriteln( 'Tile['+StrOfInt( TileY ) + ',' + StrOfInt( TileX )+']');
             while (CurrentRow < TileLength) and ((Run - Buffer) < Integer(StripSize)) do
             begin
               Pixels := ScanLine[CurrentRow+MasterRow]; { target image pointer }
               INC( Pixels, TileX*TileWidth*(BytesPerLine DIV Width ));

               if Assigned(Deprediction) then                      // depredict strip if necessary
                  Deprediction(Run, Width - 1);

               ColorManager.ConvertRow([Run], Pixels, TileWidth, $FF); // any color conversion comes last
               Inc(PChar(Run), BytesPerLine DIV TileCols );
               Inc(CurrentRow);
...
var IFDCount: Word;
begin { TTIFFGraphic.LoadFromStream }
  Handle := 0; Deprediction := nil; Decoder := nil;
  // we need to keep current stream position because all position information are relative to this one
  FBasePosition := Stream.Position;
  if NOT ReadImageProperties(Stream, 0) then
   BEGIN GraphicExError(gesInvalidImage, ['TIF/TIFF']); EXIT; END;
  ...
  with FImageProperties do
   try
     ...
     DeterminePredictionScheme;
     Decoder := CreateDecompressor( Stream );
     Progress(Self, psEnding, 0, False, FProgressRect, '');
     Progress(Self, psStarting, 0, False, FProgressRect, gesTransfering);
...
end; { TTIFFGraphic.LoadFromStream }

//----------------------------------------------------------------------------------------------------------------------
                (* disabled to allow tif to write as bitmap
